...
class ChatMainViewModel @Inject constructor(
    ...
    val searchedUsers: StateFlow<Resource<List<User>>> get() = _searchedUsers
...
//                        _user = Resource.failed("")
    ...
    fun getUsers() = viewModelScope.launch {
        getAllUsers().collect { state ->
            when(state) {
                is Resource.Loading -> {
                    _users.value = Resource.loading()
                }
                is Resource.Success -> {
                    _users.value = Resource.success(state.data)
                }
                is Resource.Failure -> {
                    Timber.e(state.errorMessage)
//                    _users = Resource.failed(state.errorMessage!!)
                }
            }
        }
    }
    ...
    fun searchUsers(userName: String) = viewModelScope.launch {
        searchUser(userName).collect { state ->
            when(state) {
                is Resource.Loading -> {
                    _users.value = Resource.loading()
                }
